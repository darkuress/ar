...
#################################################################################       
        ...
        def twistArm(self, Stretch_Arm):
                ...
                for LR in L_o_R:
                        self.armJoints = ['%sShoulder' %LR, '%sElbow' %LR, '%sWrist' %LR]
                        for i in range(len(self.armJoints)-1):
                                pos_1 = cmds.xform('%s' %self.armJoints[i], ws = True, t = True, q = True )
                                pos_2 = cmds.xform('%s' %self.armJoints[i+1], ws = True, t = True, q = True )
                                if LR == 'L_':
                                        pos_2 = [pos_2[0]-pos_1[0], pos_2[1]-pos_1[1], pos_2[2]-pos_1[2]]
                                        cmds.insertJoint('%s' %self.armJoints[i])
                                        cmds.joint('joint1', n = '%s_Twist1' %self.armJoints[i], e = True, co = True, p = [pos_1[0] + pos_2[0]/3, pos_1[1] + pos_2[1]/3, pos_1[2] + pos_2[2]/3])
                                        cmds.insertJoint('%s_Twist1' %self.armJoints[i])
                                        cmds.joint('joint1', n = '%s_Twist2' %self.armJoints[i], e = True, co = True, p = [pos_1[0] + 2*pos_2[0]/3, pos_1[1] + 2*pos_2[1]/3, pos_1[2] + 2*pos_2[2]/3])                
                                else:   
                                        pos_2 = [-pos_2[0]+pos_1[0], pos_2[1]-pos_1[1], pos_2[2]-pos_1[2]]
                                        cmds.insertJoint('%s' %self.armJoints[i])
                                        cmds.joint('joint1', n = '%s_Twist1' %self.armJoints[i], e = True, co = True, p = [pos_1[0] - pos_2[0]/3, pos_1[1] + pos_2[1]/3, pos_1[2] + pos_2[2]/3])
